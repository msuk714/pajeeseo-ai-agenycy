...
module.exports = async function pagespeed(req, res) {
  ...
  try {
    ...
    const filmstrip = (audits['screenshot-thumbnails']?.details?.items || []).slice(0, 10).map((item) => ({ timing: Number(item.timing || 0), timestamp: Number(item.timestamp || 0), data: item.data || '' })).filter((item) => item.data);
    const screenshot = audits['final-screenshot']?.details?.data || filmstrip.slice(-1)?.[0]?.data || '';
    const auditIds = [
    ...
    return send(res, 200, {
      ...
      site: { name: website?.siteName || new URL(url).hostname, url: lighthouse.finalUrl || url, favicon: website?.favicon || '', title: website?.title || '', screenshot, filmstrip },
